        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title m-2">Edit Profile</h4>
                    <div class="pull-right float-end ">
		                 <ul class="nav nav_filter ">
		                    <li class="nav-item text-end">
		                       <a href="javascript:void(0)" class="btn btn-primary mb-2 nav-profile" >Back</a>
                        ...
                        <form class="" id="update_customer_form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-row size-5">
                                ...
                                <div class="form-group col-md-3">
									...
									<option  value="1" <?php if($customer['user_type'] == '1'){ echo 'selected'; } else{ echo 'disabled';}?>>Bailer</option>
									<option  value="2" <?php if($customer['user_type'] == '2'){ echo 'selected'; } else{ echo 'disabled';}?>>End User</option>
									<option  value="3" <?php if($customer['user_type'] == '3'){ echo 'selected'; } else{ echo 'disabled';}?>>Paper Mill</option>
									<option  value="4" <?php if($customer['user_type'] == '4'){ echo 'selected'; } else{ echo 'disabled';}?>>Trader</option>
                                    <option  value="5" <?php if($customer['user_type'] == '5'){ echo 'selected'; } else{ echo 'disabled';}?>>Transporter</option>
                                    <option  value="10" <?php if($customer['user_type'] == '5'){ echo 'selected'; } else{ echo 'disabled';}?>>Other</option>
								</select>

                                    <span class="text-danger size-7 e_maximum_stock"></span>
                                </div>
                                 <input type="hidden" id="role" value="<?=$customer['user_type'];?>">
                                 <input type="hidden" name="approved" value="0">
                                <div class="form-group col-md-3">
                                    <label>Designation</label>
                                    <input type="text" class="form-control" name="designation" required value="<?=$customer['designation']?>" >
                                </div>
