...
// exit;
    ...
    public function getViewListProduct($id)
    {
        ...
        if ($deckle == 1) {
            $data["decklesize"] = $data[
                ...
                "select ops.id,ops.size,ops.quantity, type,gsm,bf  from ordered_product_specification ops left join list_specification ls on ops.product = ls.id  where ops.list_id = '$id' and ops.added_by = '$addedby' "
            );
        }
        $data["title"] = "Product ";
        ...
        if ($data["listedproduct"] !== null) {
            ...
            if ($deckle == 1) {
                $data["decklesize"] = $this->main_model->getQueryAllData(
                    "select ops.id,ops.size,ops.quantity, type,gsm,bf  from ordered_product_specification ops left join list_specification ls on ops.product = ls.id  where ops.list_id = '$id' and ops.added_by = '$this->memberID' "
                );
            }
        }
    ...
    public function getSendListResponse($id)
    {
        ...
        if ($this->accountStatus == 1) {
            ...
            // $counterid = $data['response']['counterid'];

            // $data['counteroffer'] = $this->main_model->getQueryRowData("select * from counter_offer where id = '$counterid' ");

            $addedby = $data["listedproduct"]["added_by"];
            $data["requestedsize"] = $this->main_model->getQueryAllData(
                "select ops.id,ops.size,ops.quantity, type,gsm,bf  from ordered_product_specification ops left join list_specification ls on ops.product = ls.id  where ops.list_id = '$id' and ( ops.added_by = '$this->memberID' or ops.added_by = '$addedby'  )"
            );
    ...
    public function getConfirmSaleResponse($id)
    {
        ...
        $data_notification["notification_for"] = $listid["interested_id"];
                ...
                if ($check) 
                {
                    for ($i = 0; $i < count($specification_id); $i++) 
                    {
                        ...
                        if (in_array($row_id, $checked_id)) 
                        {
                            ...
                            $notification["added_by"] = $this->memberID;
                            $notification["added_date"] = $this->currentDate;
                            $notification["notification_for"] =$list_by["added_by"]??'';
    ...
    public function submitCounteroffer()
    {
        ...
        $data_notification["description"] =
            "product counter sent from seller to buyer";
        $data_notification["added_date"] = $this->currentDate;
        $data_notification["notification_for"] = $res_id["interested_id"];
        $notification = $this->main_model->insert_table(
            "notification",
            $data_notification
        );
    ...
    public function approveCounteroffer()
    {
        ...
        if ($cnt) {
            //   $cntdata = $this->main_model->getRowData("response_specification", "size", "response_id='$id'");
            //     // print_r($cntdata);
            //     if(trim($cntdata['size'])=='X' || trim($cntdata['size'])=='x'){
            //         $returnData['success']   =   'false';
    ...
    public function getSizeList()
    {
        ...
        $data["requestedsize"] = $this->main_model->getQueryAllData(
            "select ops.id,ops.size,ops.quantity, type,gsm,bf  from ordered_product_specification ops left join list_specification ls on ops.product = ls.id  where ops.list_id = '$listid' and ops.added_by = '$userid' "
        );
